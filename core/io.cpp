...
cv::Mat ReadImageToCVMat(const string& filename,
  ...
  _img_origin = cv::imread(filename, cv_read_flag);
  ...
  static std::unordered_map<string, cv::Mat> filecache;
  ...
  if(filecache.find(filename) == filecache.end()) {	
    if(mem_free_per >= 0.1) {
#ifdef USE_MOG
      static char resolved_path[5120];
      memset(resolved_path, 0, 5120);
      realpath(filename.c_str(), resolved_path);
      int cv_read_flag = (is_color ? CV_LOAD_IMAGE_COLOR : CV_LOAD_IMAGE_GRAYSCALE);
      std::string buffer;
      int len = st::mog::GetInstance().get(std::string(resolved_path), buffer);
      ...
      filecache[filename] = cv_img_origin;
#else
      std::streampos size;
    ...
    cv_img_origin = filecache[filename];
  }
