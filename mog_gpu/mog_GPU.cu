...
void mog_malloc_gpu(int64_t device_id, int64_t slab_size, char** slabs, int64_t slab_id) {
...
char* mog_malloc_gpu(int64_t device_id, int64_t size) {
  cudaSetDevice(device_id);
  char *data_p;
...
void mog_free_gpu(int64_t device_id, char* data_p) {
  cudaSetDevice(device_id);
